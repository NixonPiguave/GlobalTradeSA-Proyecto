...
def generar_ventas(conn: duckdb.DuckDBPyConnection, cantidad: int = 100_000) -> dict[str, Any]:
    ...
    row = execute_query(
        conn,
        "SELECT COALESCE(MAX(order_id), 0), COALESCE(MAX(id_venta), 0) FROM fact_ventas",
        fetch="one",
    )
    ...
    rps = round(n / elapsed, 3) if elapsed > 0 else 0.0

    notify_db_changed(conn)
    total_ventas = int(conn.execute("SELECT COUNT(*) FROM fact_ventas").fetchone()[0])
    logger.info("Generadas %d ventas en %.3f s (%.0f reg/s)", n, elapsed, rps)
