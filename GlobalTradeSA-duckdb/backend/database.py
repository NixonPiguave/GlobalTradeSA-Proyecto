...
def init_db(db_path: str) -> None:
    ...
    with _init_lock:
        ...
        try:
            conn = duckdb.connect(str(path), read_only=_is_readonly())
...
def close_db() -> None:
    """Cierra la conexión del hilo actual y resetea la ruta (apagado del servidor)."""
    global _db_path
...
def _get_thread_connection() -> duckdb.DuckDBPyConnection:
    ...
    conn = getattr(_conn_local, "conn", None)
    local_revision = getattr(_conn_local, "revision", None)
    if conn is not None and local_revision != _db_revision:
        release_thread_connection()
        conn = None

    if conn is None:
        conn = duckdb.connect(_db_path, read_only=_is_readonly())
        try:
            conn.execute("SET threads TO 4")
        except duckdb.Error:
            pass
        _conn_local.conn = conn
        _conn_local.revision = _db_revision
    return conn


@contextmanager
def get_connection():
    """Context manager: conexión DuckDB por hilo (threading.local)."""
