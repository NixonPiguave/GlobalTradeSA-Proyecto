...
# Columnas que deben ser estrictamente positivas (unidades y precios).
POSITIVE_COLUMNS: list[str] = ["units_sold", "unit_price", "unit_cost"]
    ...
    for idx, row in df.iterrows():
        ...
        for col in NUMERIC_COLUMNS:
            ...
            try:
                ...
                continue
            if col in POSITIVE_COLUMNS and Decimal(str(value)) <= 0:
                errors.append(
                    {
                        "row": row_number,
                        "column": col,
                        "reason": f"'{col}' debe ser mayor que 0: '{value}'.",
                    }
                )
        ...
        # Validar coherencia de fechas: ship_date no puede ser anterior a order_date
        try:
            od = datetime.strptime(str(row["order_date"]).strip(), "%Y-%m-%d")
            sd = datetime.strptime(str(row["ship_date"]).strip(), "%Y-%m-%d")
            if sd < od:
                errors.append(
                    {
                        "row": row_number,
                        "column": "ship_date",
                        "reason": (
                            f"'ship_date' ({sd.date()}) es anterior a 'order_date' "
                            f"({od.date()})."
                        ),
                    }
                )
        except (ValueError, TypeError):
            pass  # ya se reportó en la validación de formato de fechas
